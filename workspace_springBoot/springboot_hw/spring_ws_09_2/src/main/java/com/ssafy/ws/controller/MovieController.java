...
public class MovieController {
    ...
    public String showList(Model model) {
        ...
        // movies.add(new Movie(1, "베놈 2: 렛 데어 비 카니지", "앤디 서키스", "액션", 97, "영화 1",
        // null));
        // movies.add(new Movie(2, "기생충", "봉준호", "드라마", 131, "영화 2", null));
        // movies.add(new Movie(3, "해리포터와 마법사의 돌", "크리스 콜럼버스", "판타지", 152, "영화 3",
        // null));
    ...
    public String doRegist(@ModelAttribute Movie movie,
            @RequestPart(value = "file", required = false) @RequestParam("file") MultipartFile file)
            throws IllegalStateException, IOException {
        if (file != null && file.getSize() > 0) {
            // 파일을 저장할 폴더 지정
            Resource res = resLoader.getResource("./resources/static/upload");
