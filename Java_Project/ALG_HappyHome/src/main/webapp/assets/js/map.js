...
function drawApts() {
  ...
  let url =
    'http://localhost:8080/HappyHome/main?action=map/rest/list&lngFrom=' +
    swLng +
    '&lngTo=' +
    neLng +
    '&latFrom=' +
  ...
  fetch(url)
    ...
    .then((jsons) => {
      ...
      jsons.forEach((json) => {
        ...
        dateTd.appendChild(
          document.createTextNode(json.dealYear + '/' + json.dealMonth + '/' + json.dealDay)
        );
...
function makeList(jsons) {
  ...
  jsons.forEach((json) => {
    ...
    tr.addEventListener('click', () =>
      viewMap(json.apartmentName, json.aptCode, json.lat, json.lng)
    );
