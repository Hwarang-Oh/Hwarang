...
public class BoardList extends HttpServlet {
	private static final long serialVersionUID = 1L; // List View의 요청도 Get 방식으로 이루어짐
	private BoardDao boardDao = new BoardDaoImpl();
	
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			List<BoardDto> list = boardDao.selectArticles();
			...
			out.println("          location.href=\"/0318_servlet_board1/view?article_no=\" + this.getAttribute(\"data-no\")");
			out.println("        });");
			out.println("      });");
			out.println("");
			out.println("      document.querySelector(\"#btn-mv-register\").addEventListener(\"click\", function () {");
			out.println("        location.href = \"/0318_servlet_board1/board/write.html\";");
			out.println("      });");
			out.println("    </script>");
			out.println("  </body>");
			out.println("</html>");
