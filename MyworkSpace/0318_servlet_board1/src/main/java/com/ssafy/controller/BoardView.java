...
@WebServlet("/view")
public class BoardView extends HttpServlet { // View의 요청은 Get 방식으로 이루어진다.
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			BoardDto boardDto = boardDao.selectArticle(articleNo);
			resp.setContentType("text/html;charset=utf-8");
			PrintWriter out = resp.getWriter();
			if(boardDto != null) {
				boardDao.UpdateHit(articleNo); // 여기에다 하는 것이 맞다. 선후관계가 맞음
