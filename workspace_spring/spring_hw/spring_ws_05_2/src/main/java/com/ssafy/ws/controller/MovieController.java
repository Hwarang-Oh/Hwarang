...
public class MovieController {
	...
	public String showList(Model model) {
		...
		movies.add(new Movie(1, "베놈 2: 렛 데어 비 카니지", "앤디 서키스", "액션", 97, "영화 1", null));
		movies.add(new Movie(2, "기생충", "봉준호", "드라마", 131, "영화 2", null));
		movies.add(new Movie(3, "해리포터와 마법사의 돌", "크리스 콜럼버스", "판타지", 152, "영화 3", null));
	...
	@PostMapping("/regist")
	public String doRegist(@ModelAttribute Movie movie,
			@RequestPart(value = "file", required = false) @RequestParam("file") MultipartFile file, Model model)
		...
		if (file != null && !file.isEmpty()) {
			...
			// Generate a new file name with timestamp to prevent duplication
			String timeStamp = new SimpleDateFormat("yyyyMMdd_HHmmss").format(new Date());
			String newFileName = timeStamp + "_" + orgFileName;

			// Get the upload directory path
			String uploadDirPath = "resources/upload";

			// Get the real path of the upload directory
			Resource uploadDirResource = resLoader.getResource(uploadDirPath);
			String realPath = uploadDirResource.getFile().getAbsolutePath();
			...
			// Set the file name in the movie object
			movie.setOrgImg(orgFileName);
			movie.setImg(newFileName);
		}
