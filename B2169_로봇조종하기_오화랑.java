...
//        StringBuilder sb2 = new StringBuilder();
        ...
        for (int i = 1 ; i <= N ; i++) {
            for (int j = 1 ; j <= M ; j++) {
                toRightMap[i][j] = fromRightMap[i][j] = finalMap[i][j] = Integer.MIN_VALUE;
            }
        }
        ...
        for (int i = 1 ; i <= N ; i++) {
            ...
            for (int j = 1; j <= M ; j++) {
                // Raw District Value Map
                Map[i][j] = Integer.parseInt(st.nextToken());
                // Memo MaxValue from toRight Cumulative Sum
                toRightMap[i][j] = Math.max(toRightMap[i][j - 1] + Map[i][j], finalMap[i - 1][j] + Map[i][j]);
            }
            if (i == 1) {
                for (int j = M ; j >= 1 ; j--)  fromRightMap[i][j] = Map[i][j];
            }
            else {
...
//                sb1.append(toRightMap[i][j]).append(" ");
//                sb2.append(fromRightMap[i][j]).append(" ");
